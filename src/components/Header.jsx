...
const Header = () => {
  const [isopen, setisopen] = useState(false) //open and colse nav menu on mobile 
  ...
  return (
      <>
        ...
        <div className=" pr-6 pl-6 sm:pl-16 sm:pr-0 flex items-center gap-16 w-[736px] h-[46px] z-20 relative  justify-between">
          <img className="w-[40px] h-[40px] sm:w-[48px] sm:h-[48px]" src="/assets/shared/logo.svg" alt="star logo " />
          <img className="toggle sm:hidden cursor-pointer" src="/assets/shared/icon-hamburger.svg" alt="hamburger toggle menu" onClick={()=>setisopen(!isopen)} />
          <img className="bg-line opacity-25 lg:block hidden" src="/assets/shared/line.svg" alt="stright line" />
        </div>
      ...
      <ul className={`${isopen ? "block" : "hidden"} mobile-nav color-nav text-white absolute top-0 right-0 z-200 w-[254px] h-[812px] pl-8 backdrop-blur-[80px]` }>
        <div className="grid gap-[45px] relative">
        <div className="pt-8 pb-8 w-[222px] h-[85px] grid justify-end relative">
          <img className="pr-6 cursor-pointer" src="/assets/shared/icon-close.svg" alt="icons close" onClick={()=>setisopen(!isopen)} />
        </div>
          <div className="w-[222px] h-[172px] gap-8 grid relative">
            {navItems.map(({ label, path, number }) => (
              <li className={`h-[19px] relative `} key={path}>
                <a className={`flex gap-3 relative ${currentPath === path ? "active-a" : ""}`} href={path} >
